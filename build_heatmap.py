...
Output:
  - OUT_DIR/codemap.tsv : path \t bytes \t lines \t commits \t bug_commits \t commits_per_kloc \t bugs_per_kloc \t bugs_per_commit
    Filtered to non-test .dart files present in current tree.
...
# Same, per Dart/Flutter package module (Code City "module mode").
...
def _district(path):
    """Dotted district for a repo-relative path (mirrors render_codecity._district).

    Dart has no src-root convention the way Java's `src/main/java/...` does, so the
    district is the directory path under `lib/` (a package's own source root), dotted;
    falling back to the immediate parent directory name outside `lib/`."""
    parts = path.split("/")
    if "lib" in parts:
        j = parts.index("lib")
        pkg = parts[j + 1:-1]
...
# ── Dart/Flutter package map ─────────────────────────────────────────────────
# A module = a directory holding a pubspec.yaml — Dart/Flutter's own package
# boundary marker, one per package in a melos-style monorepo. Each file
...
def _discover_module_dirs():
    ...
    `pubspec.yaml` under a vendored or generated directory is not a module of this repo, and
...
print(f"discovered {len(MODULE_DIRS)} Dart/Flutter package dir(s)", file=sys.stderr)
...
def _counts_toward_diagram(fp):
    """Does this repo-relative path belong in the city? Non-test, non-generated .dart.
    ...
    return repo_files.is_dart_source(fp)
...
def flush_commit():
    ...
    # ...and to each distinct Dart/Flutter package module it touched.
...
# collect current dart files (non-test) + their bytes + line counts.
...
dart_files = repo_files.dart_sources(REPO_DIR)

print(f"found {len(dart_files)} current non-test dart files", file=sys.stderr)
