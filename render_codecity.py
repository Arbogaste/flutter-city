...
def _district(path):
    ...
    if "lib" in parts:
        ...
        package_parts = parts[lib_index + 1:-1]
        if package_parts:
...
def _change_set(analyzed):
    ...
    Auto-detected, no configuration needed:
      ...
      3. else, the most recent commit that touches an analysed file — usually
         ...
         configs / non-Dart tests. "The last commit that actually changed code."
...
with TSV.open() as f:
    ...
    for row in reader:
        ...
        rows.append({
            "path": path,
            "name": path.rsplit("/", 1)[-1].replace(".dart", ""),
...
# ── Per-module rows for "module mode" (Dart/Flutter packages) ────────────────
# Same shape as a file/package row, so the same treemap machinery renders each
# Dart/Flutter package module as a building on its parent-module floor. Aggregates come
...
# `lib/src/widgets/...` appears in the coupling map about
...
.controls .spanAll { grid-column: 2 / -1; }
  ...
  /* Module + source-root prefix (dir minus the Dart package): dimmer, smaller. */
  #hover .idhdr .folder {
    color: #8b98ad;
    font-size: 10.5px;
    line-height: 1.3;
...
<section class="howto" id="howto" hidden>
  <div class="howto-card" role="dialog" aria-modal="true" aria-labelledby="howtoTitle">
    ...
    <p>This page is a self-contained snapshot of one repository. The same generators
      turn <em>any</em> folder of Dart/Flutter sources into this 3-D city &mdash; a building
      per file, grouped into districts by package. Point the pipeline at another repo and
      open the result:</p>
    <div class="howto-cmd">
    ...
    <p class="howto-note">Dart/Flutter sources only. Re-run anytime to refresh. <code>open</code> is
...
const MODULES = __MODULES_JSON__;     // per-Dart/Flutter-package-module rows (same shape) for module mode
...
// or module rows (a building per Dart/Flutter package module). Falls back to classes.
...
// The dotted Dart package of a class row lives in `district` ("root" means none).
...
// spell out its Dart package, i.e. the module + source root that precedes the
// package. e.g. path ".../my_app/lib/src/widgets/request" with package
// "widgets...request" → "my_app/lib".
...
function hoverHeaderForFile(file) {
  ...
  if (view === "classes") {
    ...
    // Title shows the real filename (foo_widget.dart), not the bare class name.
...
function formatDistrictHover(district) {
  ...
  return identityHeader(shortName, "", pkg) +
    `<ul class="props"><li><span>Dart files: <b>${count}</b></span></li></ul>`;
}
...
# No public URL is assumed here — flutter-city has none yet. Set
# FLUTTER_CITY_REPO to a real clone URL once one exists; until then the
# button shows the generic "wherever you cloned this from" instruction.
TOOL_REPO = os.environ.get("FLUTTER_CITY_REPO", "")
if TOOL_REPO:
    _get_step = f"""# 1. The generators (this page was made by them):
git clone {TOOL_REPO} ~/flutter-city
"""
    _generate_path = "~/flutter-city/generate.sh"
else:
    TOOL_REPO = "(the flutter-city checkout you're running this from)"
    _get_step = ""
    _generate_path = "./generate.sh"
BUILD_CMD = f"""{_get_step}# Point the generators at any git repo of Dart/Flutter sources — that's the whole configuration.
{_generate_path} ~/workspace/your-repo

# Open the city (macOS `open`; Linux `xdg-open`; Windows `start`).
open ~/workspace/your-repo/.flutter-city/codecity.html
"""
